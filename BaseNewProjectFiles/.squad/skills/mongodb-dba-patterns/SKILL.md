...
Expert DBA administration patterns for MongoDB 7.x+ as used in {ProjectName}. Covers cluster management,
...
- Creating/dropping databases or collections for {ProjectName} entities (e.g., {CollectionName}s)
...
**Key principles:**
- {ProjectName} uses `MongoDB.EntityFrameworkCore`, which requires a replica set for multi-document transactions
...
await database.CreateCollectionAsync("{CollectionName}s", new CreateCollectionOptions
{
...
- Define collection names in your DbContext (e.g., `{CollectionName}s`)

### Backup and Restore

```bash
# Full database backup
mongodump --uri="mongodb://localhost:27017" --db={DbName} --out=/backup/$(date +%F)

# Restore from backup
mongorestore --uri="mongodb://localhost:27017" --db={DbName} /backup/2026-03-03/{DbName}

# Single-collection backup
mongodump --uri="mongodb://localhost:27017" --db={DbName} --collection={CollectionName}s --out=/backup/collection
```
...
db.{CollectionName}s.createIndex({ "Author.Name": 1, "CreatedAt": -1 }, { name: "idx_author_created" })

// Partial index for non-archived documents (mirrors the Archived filter in the repository)
db.{CollectionName}s.createIndex({ "CreatedAt": -1 }, { partialFilterExpression: { "Archived": false }, name: "idx_active_created" })
...
// Explain a query
db.{CollectionName}s.find({ "Archived": false }).sort({ "CreatedAt": -1 }).explain("executionStats")
```
...
- Apply the principle of least privilege: the app user should have `readWrite` only on `{DbName}`
...
- MongoDB 7.0 is the recommended minimum for {ProjectName} production deployments
